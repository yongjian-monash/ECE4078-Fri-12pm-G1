...
class Operate:
    ...
    def drive_to_point(self, waypoint, canvas):
        ...
        if abs(turn_diff) <= 30*np.pi/180:
            turn_offset = 0.035
        elif abs(turn_diff) <= 60*np.pi/180:
            turn_offset = 0.020
        elif abs(turn_diff) <= 90*np.pi/180:
            turn_offset = 0.008
        elif abs(turn_diff) <= 120*np.pi/180:
            turn_offset = 0.005
        elif abs(turn_diff) <= 150*np.pi/180:
            turn_offset = 0.002
        elif abs(turn_diff) <= 180*np.pi/180:
            turn_offset = 0.000
        ...
        if turn_diff > 0.0: # turn left
            ...
            turn_time = (abs(turn_diff)*baseline*1.12)/(2.0*scale*wheel_vel)
            lv, rv = self.pibot.set_velocity([0, -1], turning_tick=wheel_vel, time=turn_time+turn_offset)
            turn_drive_meas = measure.Drive(lv, rv, turn_time/1.12)
